    ...
    def make
      ...
      begin
        ...
        if not @compress.blank?
          success = Paperclip.run("jpegoptim", "--max=85 --strip-all :dest", :dest => File.expand_path(dst.path))
          dst = File.open(File.expand_path(dst.path))
        else
          success = Paperclip.run("jpegoptim", "--max=85 :dest", :dest => File.expand_path(dst.path))
          dst = File.open(File.expand_path(dst.path))
        end
