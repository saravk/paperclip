    ...
    def make
      ...
      begin
        ...
        if not @compress.blank?
          success = Paperclip.run("jpegoptim", "--max=65 --strip-all :dest", :dest => File.expand_path(dst.path))
          dst = File.open(File.expand_path(dst.path))
        else
          success = Paperclip.run("jpegoptim", "--max=75 :dest", :dest => File.expand_path(dst.path))
          dst = File.open(File.expand_path(dst.path))
        end
